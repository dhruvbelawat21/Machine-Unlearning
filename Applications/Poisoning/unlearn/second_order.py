...
def second_order_unlearning(model_folder, poisoned_filename, repaired_filename, model_init, data, y_train_orig, delta_idx,
    ...
    with open(model_folder.parent.parent/'clean'/'train_results.json', 'r') as f:
...
def main(model_folder, config_file, verbose):
    ...
    poison_kwargs = Config.from_json(os.path.join(model_folder, 'poison_config.json'))
...
#     model_init = lambda: load_model(os.path.join(model_folder, "poisoned_model.hdf5"))

#     poisoned_filename = 'poisoned_model.hdf5'
#     repaired_filename = 'repaired_model.hdf5'

#     second_order_unlearning(
#         model_folder, poisoned_filename, repaired_filename,
#         model_init, data, y_train_orig, injector.injected_idx,
#         unlearn_kwargs, verbose=verbose
#     )


# def second_order_unlearning(model_folder, poisoned_filename, repaired_filename, model_init, data,
#                             y_train_orig, delta_idx, unlearn_kwargs, order=2, verbose=False):
#     unlearning_result = UnlearningResult(model_folder)
#     poisoned_weights = os.path.join(model_folder, poisoned_filename)
#     log_dir = model_folder

#     # --- REMOVED dependency on clean baseline ---
#     clean_acc = None  # no baseline accuracy available
...
#     unlearn_kwargs['order'] = order
#     acc_before, acc_after, diverged, logs, unlearning_duration_s, params = evaluate_unlearning(
#         model_init, poisoned_weights, data, delta_idx, y_train_orig,
#         unlearn_kwargs, clean_acc=None,
#         repaired_filepath=repaired_filepath, verbose=verbose,
#         cm_dir=cm_dir, log_dir=log_dir
#     )

#     # If clean_acc is not available, skip restored % calculation
#     if clean_acc:
#         acc_perc_restored = (acc_after - acc_before) / (clean_acc - acc_before)
#     else:
#         acc_perc_restored = None
